...
// In production: set to your Render URL e.g. 'https://rio-ai.onrender.com'
const API_BASE = '';
...
// Format text with markdown-style formatting (bold, bullets, numbers, etc.)
function formatMessageText(text) {
    ...
    html = html.replace(/(<li>.*?<\/li>)/s, (match) => {
        const hasWrapper = match.startsWith('<ul>');
        return hasWrapper ? match : '<ul>' + match + '</ul>';
    });
    ...
    html = html.replace(/(<li class="numbered">.*?<\/li>)/s, (match) => {
        const hasWrapper = match.startsWith('<ol>');
        return hasWrapper ? match : '<ol>' + match + '</ol>';
    });
...
function appendMessage(text, sender, isError = false, correction = null) {
    ...
    if (sender === 'agent') {
        const parts = text.split(/(?:\*\*Answer\*\*|Answer:|Here's a possible answer|Here is a possible answer|Possible answer:)/i);
        if (parts.length >= 2) {
            const qBlock = document.createElement('div');
            qBlock.className = 'qa-block question-block';
            qBlock.textContent = parts[0].trim();
            bubble.appendChild(qBlock);

            const aBlock = document.createElement('div');
            aBlock.className = 'qa-block answer-block';
            const answerText = (parts.slice(1).join('')).trim();
            aBlock.innerHTML = formatMessageText(answerText);
...
function createSpeechRecognition() {
    ...
    sr.onerror = (event) => {
        ...
        if (event.error === 'not-allowed') {
            appendMessage('Microphone access denied. Please allow microphone permissions in your browser and try again.', 'agent', true);
        }
        isRecording = false;
        ...
        speechRecognition = null; // Force re-creation on next attempt
    };
...
function startRecording() {
    ...
    // Re-create SpeechRecognition if it was nulled by a previous error
    ...
    if (speechRecognition) {
        ...
        try {
            ...
            console.warn('Speech recognition start() failed, retrying with fresh instance', err);
            speechRecognition = createSpeechRecognition();
            if (speechRecognition) {
                try {
                    speechRecognition.start();
                } catch (err2) {
                    console.warn('Speech recognition retry also failed, falling back to WAV', err2);
